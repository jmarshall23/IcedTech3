...
void idActor::NativeEvent_EnableEyeFocus() {
	allowEyeFocus = true;
	blink_time = gameLocal.time + blink_min + gameLocal.random.RandomFloat() * (blink_max - blink_min);
}

void idActor::NativeEvent_DisableEyeFocus() {
	allowEyeFocus = false;
	idEntity* headEnt = head.GetEntity();
	if (headEnt) {
...
void idActor::NativeEvent_Footstep() {
	PlayFootStepSound();
}

void idActor::NativeEvent_EnableWalkIK() {
	walkIK.EnableAll();
}

void idActor::NativeEvent_DisableWalkIK() {
	walkIK.DisableAll();
}

void idActor::NativeEvent_EnableLegIK(int num) {
	walkIK.EnableLeg(num);
}

void idActor::NativeEvent_DisableLegIK(int num) {
	walkIK.DisableLeg(num);
}

void idActor::NativeEvent_PreventPain(float duration) {
	painTime = gameLocal.time + SEC2MS(duration);
}

void idActor::NativeEvent_DisablePain() {
	allowPain = false;
}

void idActor::NativeEvent_EnablePain() {
	allowPain = true;
}

const char* idActor::NativeEvent_GetPainAnim() {
	if (!painAnim.Length()) {
		return "pain";
	}
	else {
...
void idActor::NativeEvent_SetAnimPrefix(const char* prefix) {
	animPrefix = prefix;
}

void idActor::NativeEvent_StopAnim(int channel, int frames) {
	switch (channel) {
	case ANIMCHANNEL_HEAD:
...
int idActor::NativeEvent_PlayAnim(int channel, const char* animname) {
	animFlags_t flags;
	idEntity* headEnt;
...
int idActor::NativeEvent_PlayCycle(int channel, const char* animname) {
	animFlags_t flags;
	int anim = GetAnim(channel, animname);
	if (!anim) {
		if ((channel == ANIMCHANNEL_HEAD) && head.GetEntity()) {
...
int idActor::NativeEvent_IdleAnim(int channel, const char* animname) {
	int anim = GetAnim(channel, animname);
	if (!anim) {
		if ((channel == ANIMCHANNEL_HEAD) && head.GetEntity()) {
			gameLocal.DPrintf("missing '%s' animation on '%s' (%s)\n", animname, name.c_str(), spawnArgs.GetString("def_head", ""));
...
void idActor::NativeEvent_SetSyncedAnimWeight(int channel, int anim, float weight) {
	idEntity* headEnt = head.GetEntity();
	switch (channel) {
	case ANIMCHANNEL_HEAD:
		if (headEnt) {
...
void idActor::NativeEvent_OverrideAnim(int channel) {
	switch (channel) {
	case ANIMCHANNEL_HEAD:
		headAnim.Disable();
		if (!torsoAnim.IsIdle()) {
...
void idActor::NativeEvent_EnableAnim(int channel, int blendFrames) {
	switch (channel) {
	case ANIMCHANNEL_HEAD:
...
void idActor::NativeEvent_SetBlendFrames(int channel, int blendFrames) {
	switch (channel) {
	case ANIMCHANNEL_HEAD:
...
int idActor::NativeEvent_GetBlendFrames(int channel) {
	switch (channel) {
	case ANIMCHANNEL_HEAD:
...
void idActor::NativeEvent_AnimState(int channel, const char* statename, int blendFrames) {
	SetAnimState(channel, statename, blendFrames);
}

const char* idActor::NativeEvent_GetAnimState(int channel) {
	return GetAnimState(channel);
}

bool idActor::NativeEvent_InAnimState(int channel, const char* statename) {
	return InAnimState(channel, statename);
}

void idActor::NativeEvent_FinishAction(const char* actionname) {
	if (waitState == actionname) {
		SetWaitState("");
	}
}

bool idActor::NativeEvent_AnimDone(int channel, int blendFrames) {
	switch (channel) {
	case ANIMCHANNEL_HEAD:
...
float idActor::NativeEvent_HasAnim(int channel, const char* animname) {
	return (GetAnim(channel, animname) != NULL) ? 1.0f : 0.0f;
}

void idActor::NativeEvent_CheckAnim(int channel, const char* animname) {
...
const char* idActor::NativeEvent_ChooseAnim(int channel, const char* animname) {
	int anim = GetAnim(channel, animname);
	if (anim) {
		if (channel == ANIMCHANNEL_HEAD) {
			if (head.GetEntity()) {
...
float idActor::NativeEvent_AnimLength(int channel, const char* animname) {
	int anim = GetAnim(channel, animname);
	if (anim) {
		if (channel == ANIMCHANNEL_HEAD) {
			if (head.GetEntity()) {
...
float idActor::NativeEvent_AnimDistance(int channel, const char* animname) {
	int anim = GetAnim(channel, animname);
	if (anim) {
		if (channel == ANIMCHANNEL_HEAD) {
			if (head.GetEntity()) {
...
bool idActor::NativeEvent_HasEnemies() {
	return HasEnemies();
}

idActor* idActor::NativeEvent_NextEnemy(idEntity* ent) {
	idActor* actor;
	if (!ent || (ent == this)) {
...
idActor* idActor::NativeEvent_ClosestEnemyToPoint(const idVec3& pos) {
	return ClosestEnemyToPoint(pos);
}

void idActor::NativeEvent_StopSound(int channel, int netSync) {
...
void idActor::NativeEvent_SetNextState(const char* name) {
...
void idActor::NativeEvent_SetState(const char* name) {
	idealState = GetScriptFunction(name);
	if (idealState == state) {
		state = NULL;
	}
...
const char* idActor::NativeEvent_GetState() {
	if (state) {
		return state->Name();
	}
	else {
...
idEntity* idActor::NativeEvent_GetHead() {
