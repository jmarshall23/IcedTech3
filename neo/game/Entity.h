...
public:
	const char* NativeEvent_GetName() const;
	void NativeEvent_SetName(const idStr& newName);
	void NativeEvent_FindTargets();
	void NativeEvent_ActivateTargets(idEntity* activator);
	int NativeEvent_NumTargets();
	idEntity* NativeEvent_GetTarget(int index);
	idEntity* NativeEvent_RandomTarget(const char* ignore);
	void NativeEvent_BindToJoint(idEntity* master, const char* jointname, bool orientated);
	void NativeEvent_RemoveBinds();
	void NativeEvent_Bind(idEntity* master);
	void NativeEvent_BindPosition(idEntity* master);
	void NativeEvent_Unbind();
	void NativeEvent_SpawnBind();
	void NativeEvent_SetOwner(idEntity* owner);
	void NativeEvent_SetModel(const char* modelname);
	void NativeEvent_SetSkin(const char* skinname);
	float NativeEvent_GetShaderParm(int parmnum);
	void NativeEvent_SetShaderParm(int parmnum, float value);
	void NativeEvent_SetShaderParms(float parm0, float parm1, float parm2, float parm3);
	void NativeEvent_SetColor(float red, float green, float blue);
	idVec3 NativeEvent_GetColor();
	bool NativeEvent_IsHidden();
	void NativeEvent_Hide();
	void NativeEvent_Show();
	void NativeEvent_CacheSoundShader(const char* soundName);
	int NativeEvent_StartSoundShader(const char* shaderName, int channel);
	int NativeEvent_StartSound(const char* soundName, int channel, int netSync);
	void NativeEvent_StopSound(int channel, int netSync);
	void NativeEvent_FadeSound(int channel, float to, float over);
	idVec3 NativeEvent_GetWorldOrigin();
	void NativeEvent_SetWorldOrigin(const idVec3& pos);
	idVec3 NativeEvent_GetOrigin();
	void NativeEvent_SetOrigin(const idVec3& pos);
	idAngles NativeEvent_GetAngles();
	void NativeEvent_SetAngles(const idAngles& ang);
	idVec3 NativeEvent_GetLinearVelocity();
	void NativeEvent_SetLinearVelocity(const idVec3& velocity);
	idVec3 NativeEvent_GetAngularVelocity();
	void NativeEvent_SetAngularVelocity(const idVec3& velocity);
	idVec3 NativeEvent_GetSize();
	void NativeEvent_SetSize(const idVec3& mins, const idVec3& maxs);
	idVec3 NativeEvent_GetMins();
	idVec3 NativeEvent_GetMaxs();
	bool NativeEvent_Touches(idEntity* ent);
	void NativeEvent_SetGuiParm(const char* key, const char* val);
	void NativeEvent_SetGuiFloat(const char* key, float val);
	const char* NativeEvent_GetNextKey(const char* prefix, const char* lastMatch);
	void NativeEvent_SetKey(const char* key, const char* value);
	const char* NativeEvent_GetKey(const char* key);
	int NativeEvent_GetIntKey(const char* key);
	float NativeEvent_GetFloatKey(const char* key);
	idVec3 NativeEvent_GetVectorKey(const char* key);
	idEntity* NativeEvent_GetEntityKey(const char* key);
	void NativeEvent_RestorePosition();
	void NativeEvent_UpdateCameraTarget();
	float NativeEvent_DistanceTo(idEntity* ent);
	float NativeEvent_DistanceToPoint(const idVec3& point);
	void NativeEvent_StartFx(const char* fxName);
	bool NativeEvent_HasFunction(const char* name);
	void NativeEvent_CallFunction(const char* name);
	void NativeEvent_SetNeverDormant(bool neverDormant);
};
